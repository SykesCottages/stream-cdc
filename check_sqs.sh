...
while true; do
    ...
    MESSAGES=$(aws --endpoint-url="$ENDPOINT_URL" sqs receive-message --queue-url "$SQS_QUEUE_URL" --max-number-of-messages 10 --attribute-names All --message-attribute-names All)
    QUEUE_SIZE=$(aws --endpoint-url="$ENDPOINT_URL" sqs get-queue-attributes --queue-url "$SQS_QUEUE_URL" --attribute-names ApproximateNumberOfMessages | jq -r '.Attributes.ApproximateNumberOfMessages')
    ...
    if [ -z "$MESSAGES" ] || [ "$MESSAGES" = "{}" ]; then
        ...
        echo "$MESSAGES" | jq -r '.Messages[].ReceiptHandle' | while read -r HANDLE; do
            aws --endpoint-url="$ENDPOINT_URL" sqs delete-message --queue-url "$SQS_QUEUE_URL" --receipt-handle "$HANDLE"
