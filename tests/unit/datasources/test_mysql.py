...
def test_fetch_actual_settings(mock_cursor):
    ...
    assert all(param in query_params for param in [
        "binlog_format",
        "binlog_row_metadata",
        "binlog_row_image",
        "gtid_mode",
...
def test_mysql_datasource_init_missing_values():
    ...
    with patch.dict(os.environ, {"DB_HOST": "localhost", "DB_USER": "user"}, clear=True):
        # Missing password
        with pytest.raises(ConfigurationError) as exc_info:
            MySQLDataSource()
...
def test_connect():
    ...
    with patch('stream_cdc.datasources.mysql.BinLogStreamReader') as mock_binlog_reader:
        ...
        with patch.object(data_source, '_validate_settings'):
            ...
            connection_settings = mock_binlog_reader.call_args[1]["connection_settings"]
...
def test_connect_validation_failure():
    ...
    with patch.object(data_source, '_validate_settings', side_effect=ConfigurationError("Validation failed")):
        with pytest.raises(ConfigurationError) as exc_info:
            data_source.connect()
...
def test_connect_client_error():
    ...
    # First, mock the BinLogStreamReader class at the module level
    with patch('stream_cdc.datasources.mysql.BinLogStreamReader', side_effect=Exception("Client error")):
        # Then create the data source
        data_source = MySQLDataSource(
            host="localhost",
            user="testuser",
...
def test_listen_with_different_event_types(mysql_data_source):
    ...
    update_event.rows = [({"id": 1, "name": "Old User"}, {"id": 1, "name": "Updated User"})]
    ...
    mock_binlog_client.__iter__.return_value = [write_event, update_event, delete_event]
...
def test_listen_error(mysql_data_source):
    ...
    assert "Error while listening to MySQL binlog: Iterator error" in str(exc_info.value)
