...
while true; do
    ...
    MESSAGES=$(aws --endpoint-url="$ENDPOINT_URL" --region $REGION sqs receive-message \
        --queue-url "$SQS_QUEUE_URL" \
        --max-number-of-messages 10 \
        --attribute-names All \
        --message-attribute-names All)

    QUEUE_SIZE=$(aws --endpoint-url="$ENDPOINT_URL" --region $REGION sqs get-queue-attributes \
        --queue-url "$SQS_QUEUE_URL" \
    ...
    if [ -z "$MESSAGES" ] || [ "$MESSAGES" = "{}" ]; then
        ...
        # Process and display each message with its attributes
        echo "$MESSAGES" | jq -r '.Messages[] | "Message ID: \(.MessageId)\nBody: \(.Body)\nAttributes: \(.MessageAttributes | @json)\n-----------------"'

        # Delete processed messages
        echo "$MESSAGES" | jq -r '.Messages[].ReceiptHandle' | while read -r HANDLE; do
            aws --endpoint-url="$ENDPOINT_URL" --region $REGION sqs delete-message \
                --queue-url "$SQS_QUEUE_URL" \
