...
class MySQLDataSource(DataSource):
    ...
    def _create_binlog_client(self) -> BinLogStreamReader:
        ...
        if self.start_position not in (None, ""):
            start_position = self._fomart_gtid(self.start_position)
    ...
    def _fomart_gtid(self, gtid: str) -> str:
        """
        ...
        current_poistion = f"{server_uuid}:1-{gtid_position}"

        logger.error(f"Formatting GTID: {gtid}")
        with self.conn.cursor() as cursor:
        ...
        try:
            for event in self.client:
                ...
                if isinstance(event, GtidEvent):
                    ...
                    self.transaction_complete = False
                    logger.debug(
                        f"New transaction GTID: {self.current_position} - "
                        f"{event.dump()}"
                    )
                    continue
